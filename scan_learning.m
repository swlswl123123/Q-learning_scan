...
for ll = 1:Ne 
    ...
    for t = 1:time_slot
        ...
        for i = 1:N
            ...
            Q_learning_table{i}(scan_seq(patten(i), mod(t-1, sector)+1), tri_table(i)) = (1-alpha) * Q_learning_table{i}(scan_seq(patten(i), mod(t-1, sector)+1), tri_table(i)) + alpha * (r + gamma*max(Q_learning_table{i}(scan_seq(patten_n(i), mod(t, sector)+1), :)));
        end
