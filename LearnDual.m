function time_rec_mean = LearnDual(D, X, Y, sector)
...
for ll = 1:Ne 
    ...
    for t = 1:time_slot
        ...
        P1 = repmat(tri_table-1, [N,1]);
        P2 = repmat((tri_table-1)', [1,N]);
        Pt = P1 + P2;
        Pt(Pt==2)=0;
        Pt = Pt + eye(size(Pt));
        % 收发模式对准
        F1 = repmat(p_table-1, [N,1]);
        F2 = repmat((p_table-1)', [1,N]);
        F = F1 + F2;
        F(F==2)=0;
        F = F + eye(size(F));
        % 环境反馈
        ...
        for i = 1:N
            ...
            Q_learning_table{i}(mod(t-1, sector)+1, tri_table(i)) = (1-alpha) * Q_learning_table{i}(mod(t-1, sector)+1, tri_table(i)) + alpha * (r + gamma*max(Q_learning_table{i}(mod(t, sector)+1, :)));
        end
